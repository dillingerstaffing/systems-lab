...
/* Throughput benchmark: udiv7_shiftadd over a deterministic full-range
 * 32-bit stream (x += golden ratio each step, one add of loop overhead),
 * timed with CLOCK_MONOTONIC, best of 5 runs. Results are checksummed into
...
int main(void)
{
    ...
    for (rep = 0; rep < 5; rep++) {
        ...
        if (best_ns == 0 || ns < best_ns)
            best_ns = ns;
        printf("rep %d: %llu ns total, %.3f ns/value, sink=0x%08x\n",
               rep, (unsigned long long)ns,
               (double)ns / (double)N, sink);
    }
