/*
 * test_mullo.c: differential test of mullo64() against the native
 * 64-bit product.
 *
 * The reference is simply `a * b`: unsigned 64-bit multiplication in C
 * wraps modulo 2^64 by the standard, so it is exactly the low word of
 * the product.  No 128-bit type appears anywhere in the test or the
 * implementation under test (mullo_split.h).
 */
#include <inttypes.h>
#include <stdint.h>
#include <stdio.h>
...
#include "mullo_split.h"

#ifndef BUILD_NAME
#define BUILD_NAME "?"
#endif

/* Exact reference: native unsigned 64-bit multiply wraps mod 2^64. */
static uint64_t ref_mullo(uint64_t a, uint64_t b)
{
    return a * b;
}
...
{
    uint64_t got = mullo64(a, b);
    uint64_t want = ref_mullo(a, b);
    fnv_feed(got);
    cases++;
    if (got != want) {
        mismatches++;
 ...
 * and values chosen to force the reachable carry states of the cross
 * term: p1 + p2 wraps modulo 2^64 via (UINT64_MAX, UINT64_MAX), and
 * the top bit of the cross term is discarded by the << 32 shift in
 * cases like (0x80000000FFFFFFFF, 0x80000001FFFFFFFF). */
 ...
 * only mullo64 calls plus an xor into a volatile sink (the sink keeps
...
static void test_throughput(void)
{
    ...
    for (int pass = 0; pass < 5; pass++) {
        ...
        for (uint32_t i = 0; i < NPAIRS; i++)
            sink ^= mullo64(tpairs[2 * i], tpairs[2 * i + 1]);
        double t1 = now_ns();
...
int main(void)
{
    printf("mullo64 differential test, build %s\n", BUILD_NAME);
