...
- **lab/64**: branchless median of three int64s as a 3-element sorting network, each compare-swap from a comparison mask plus bitwise selection (sign-flip with 32-bit-half comparison so the difference can never wrap, exact over the full int64 range); 21,777,945 differential checks (16,777,216 exhaustive 8-bit triples + 729 directed 64-bit edge triples incl. INT64_MIN/INT64_MAX + 5,000,000 fixed-seed splitmix64 random 64-bit triples), 0 mismatches vs comparison-based 3-sort reference, FNV-1a checksum 0x14fbc28750757f72 identical across -O0/-O2/ASan+UBSan, no conditional jumps (0 j* instructions in the -O2 object), 10.379 ns/triple at -O2, zero warnings under -Wall -Wextra -Werror, zero sanitizer findings
...
- **lab/76**: ceiling log2 of a 64-bit word from the fill-cascade and power-of-two identities (no builtin, no floating point); 10,065,744 differential checks, 0 mismatches vs naive reference, FNV-1a checksum 0x52130d09b192774b identical across builds, 6.047 ns/value at -O2
- **lab/77**: bit span (position of highest set bit minus lowest set bit) from the fill-cascade floor and de Bruijn ctz identities; 10,065,685 differential checks, 0 mismatches vs naive reference, FNV-1a checksum 0x7c36594632d8db46 identical across builds, 7.210 ns/value at -O2
- **lab/78**: branchless per-lane unsigned 16-bit max of two packed words in one 64-bit word from the guarded-subtraction comparison identity (one guard bit per lane, minuend lane bounds prove no borrow ever leaves a lane, zero crosstalk); 4,309,161,635 differential checks (2^32 exhaustive lane pairs replicated to all four lanes + 4,194,304 directed crosstalk vectors + 21 anchors + 10,000,014 horizontal cases), 0 mismatches vs per-lane scalar reference, FNV-1a checksum 0x64d1c937981935ce identical across -O0/-O2/ASan+UBSan, horizontal max 3.943 ns/value (253.6 M values/s) at -O2, zero warnings under -Wall -Wextra -Werror, zero sanitizer findings
- **lab/79**: CRC-16/CCITT-FALSE (poly 0x1021 MSB-first, init 0xFFFF) from the shift-register recurrence, with a second table-driven implementation over a 256-entry table derived at startup from the polynomial (table proven by an independent re-derivation, 256/256 match); 1,000,256 differential messages (128,062,115 bytes), 0 mismatches; "123456789" -> 0x29B1 matches the CRC catalogue check value, empty message -> 0xFFFF; FNV-1a checksum a2efc13368379e59 identical across -O0/-O2/ASan+UBSan, zero warnings under -Wall -Wextra -Werror, zero sanitizer findings; measured at -O2: bitwise 70.8 MiB/s, table-driven 197.9 MiB/s
- **lab/81**: two's-complement 64-bit negation built only from the ~x + 1 identity (no unary minus in the implementation); 1,065,537 checks (65,536 exhaustive 16-bit inputs + 1,000,000 fixed-seed splitmix64 random 64-bit values + INT64_MIN row), 0 mismatches vs unary minus, 0 invariant violations ((x + neg(x)) == 0 mod 2^64 and neg(neg(x)) == x on every case); INT64_MIN wraps to itself, pinned by contract; FNV-1a checksum 0x6f25fc9c8181575a identical across -O0/-O2/ASan/UBSan, zero warnings under -Wall -Wextra -Werror, zero sanitizer findings; measured at -O2: 0.594 ns/value (PRNG pre-generated, excluded from the timed loop)
- **lab/82**: CRC-32 residue invariant from the GF(2) polynomial-division identity: crc32(M) appended in little-endian wire order then re-checksummed always equals 0x2144DF1C, using the lab/04 implementation as oracle (big-endian append correctly yields a different residue, documented as contract); 1,000,000 fixed-seed splitmix64 buffers (seed 0x123456789ABCDEF0, lengths 0..256, 3,868 length-0 cases), 0 mismatches, known-answer "123456789" -> 0xCBF43926 and empty message -> residue holds; FNV-1a checksum 0x277d10c8b7ea96ec identical across -O0/-O2/ASan+UBSan, zero warnings under -std=c11 -Wall -Wextra -Werror, zero sanitizer findings; measured at -O2: 207.7 MiB/s for the two-pass flow
- **lab/83**: count-trailing-zeros via popcount((x ^ (x-1)) >> 1) with hand-rolled SWAR popcount, no builtin ctz wrappers; 65,535 nonzero 16-bit inputs exhaustive + 64 single-bit words + 1,000,000 seed-fixed 64-bit values, 0 mismatches vs shift-loop reference; FNV-1a checksum 0xdd36dc676b59b047 identical across -O0/-O2/ASan+UBSan, zero warnings, zero sanitizer findings, disassembly contains no tzcnt/bsf/popcnt; measured at -O2: 3.00 ns/value
- **lab/84**: round uint64 up to the next power of two from the shift/OR fill cascade on (x-1) then +1, unsigned arithmetic, no builtins; 194 dedicated contract rows (0 -> 0, all 2^k identity for k=0..63, 2^k-1 and 2^k+1 boundaries, >2^63 wrap-to-0 rows) plus exhaustive 16-bit inputs plus 1,000,000 fixed-seed splitmix64 values, 0 mismatches vs independent division-loop reference, FNV-1a 0x45777d315651eb4c identical across -O0/-O2/ASan+UBSan, zero warnings under -std=c11 -Wall -Wextra -Werror, zero sanitizer findings, -O2 disassembly shows the intended construction (sub, five shr/or pairs, add, ret) with no tzcnt/bsf/popcnt/lzcnt; measured at -O2: 2.27 ns/value
- **lab/96**: isolate the lowest set bit of a 64-bit word via the two's-complement identity iso(x) = x & -x with the negation as unsigned (0 - x) mod 2^64, no builtins, no intrinsics, no tables; 1,065,536 differential checks (65,536 exhaustive 16-bit inputs + 1,000,000 fixed-seed splitmix64 64-bit values, seed 0x243F6A8885A308D3) against a naive per-bit scanning reference, 0 mismatches; on all 1,065,535 nonzero cases the invariants popcount(x - iso) == popcount(x) - 1 and iso & (iso - 1) == 0 held with 0 failures (independent naive bit-loop popcount, no builtins), x = 0 returns 0 per contract; FNV-1a checksum 6c73cad58aa2174f identical across -O0/-O2/ASan+UBSan, zero warnings under -std=c11 -Wall -Wextra -Werror, zero sanitizer findings; gcc 13.3 -O2 disassembly is exactly mov/neg/and/ret; measured at -O2: 2.169 ns/value (PRNG pre-generated, excluded from the timed loop)
- **lab/103**: single-pass signed divmod by 2^k from the sign-bias identity (bias = (x >> 63) & ((1ULL << k) - 1), quotient = (x + bias) >> k, remainder = x - (quotient << k)); 4,129,461 differential checks (65,536 exhaustive int16 values for k=1..63 + 64-bit edge values) vs C / and %, 0 mismatches; invariants q*2^k+r==x in __int128, sign agreement, |r|<2^k held on every case; -O2 objdump contains no div/idiv; FNV-1a checksum b9bffe669bde5c90 identical across -O0/-O2/ASan+UBSan
- **lab/13**: in-place swap of two 64-bit words via three xor-assignments (a^=b; b^=a; a^=b), no temporary; 4,295,967,296 differential checks per binary vs temp-variable reference (exhaustive 16-bit pairs + 1M random), 0 mismatches across -O0/-O2/ASan+UBSan; a==b aliasing contract (zeroes the word) verified on 6 rows; best 5.144 ns/pair
- **lab/97**: 32-bit parity via the 0x6996 nibble-parity constant (fold x to 4 bits with XOR shifts, index the constant); 2,065,536 checks (1,065,536 differential vs naive per-bit loop + 1,000,000 homomorphism), 0 mismatches; constant bit layout derived by hand in PROOF.md; 3.44 ns/value at -O2
- **lab/106**: two-word (128-bit) unsigned subtraction with borrow propagation, each stage from the borrow-out identity t = b + borrow_in (wrapping), borrow = (a < t) || (borrow_in && (t < b)), where the (t < b) term is the wrap detector for the b == UINT64_MAX edge; no __int128 in the implementation; 8,599,934,628 differential checks (36 directed edge rows, exhaustive 16-bit (a_lo, b_lo) pairs x borrow_in 0/1 = 8,589,934,592 cases, 10,000,000 fixed-seed splitmix64 128-bit cases, seed 0x123456789ABCDEF0) vs an unsigned __int128 exact-arithmetic oracle, 0 mismatches; FNV-1a checksum fce86f5578b4ca16 identical across -O0/-O2/ASan+UBSan, zero warnings under -std=c11 -Wall -Wextra -Werror, zero sanitizer findings; gcc 13.3 -O2 disassembly is exactly add/setb/and/or/sub/ret with no wide arithmetic; measured at -O2: 4.55 ns/value (PRNG pre-generated, excluded from the timed loop)
- **lab/87**: branchless sign of signed 64-bit (-1, 0, +1) via sign-shift identity ((x >> 63) | ((uint64_t)(-x) >> 63)); 1,065,541 differential checks vs comparison reference, 0 mismatches; -O2 disassembly has no conditional jump; 2.22 ns/value
- **lab/88**: two-word 128-bit increment with carry cascade from the carry identity (carry_out = (lo + 1 == 0)); 4,295,967,301 differential checks vs unsigned __int128, 0 mismatches; checksum e7c096581363da96; 4.85 ns/value at -O2
- **lab/89**: branchless per-lane absolute value of four packed signed 16-bit lanes in one 64-bit word from the sign-mask identity (x ^ m) - m, with one guard bit per lane (minuend/subtrahend lane bounds prove no borrow ever leaves a lane, zero crosstalk); 11,310,948 differential checks (all 2^16 lane values x all four lane positions plus neighbor-pattern sweeps = 1,310,720 cases, 180 directed crosstalk vectors, 48 hand-checked anchors, 10,000,000 fixed-seed splitmix64 words), 0 mismatches vs an independent scalar if/else per-lane reference; abs(-32768) = -32768 pinned by contract; abs(abs(x)) == abs(x) and per-lane nonnegativity hold on every case; FNV-1a checksum 0xd2d96df3ef5780e2 identical across -O0/-O2/ASan+UBSan, zero warnings under -std=c11 -Wall -Wextra -Werror, zero sanitizer findings; gcc 13.3 -O2 disassembly of the per-lane path contains 0 conditional jump instructions and no cmov; measured at -O2: 1.210 ns/value (826.673 M values/s, PRNG pre-generated, excluded from the timed loop)
- **lab/91**: unsigned 32-bit division by 3 via the modular-inverse identity q = (x * 0xAAAAAAAB) >> 33, no division operator in the implementation (M = 0xAAAAAAAB = (2^33 + 1) / 3; exactness derived in PROOF.md from x = 3a + r, with an independent Python cross-check); 1,065,555 differential checks (19 directed edge rows + 65,536 exhaustive 16-bit inputs + 1,000,000 fixed-seed splitmix64 32-bit values, seed 0x123456789ABCDEF0) against native / 3, 0 mismatches; FNV-1a checksum b2e1fc5468e18c88 identical across -O0/-O2/ASan+UBSan, zero warnings under -std=c11 -Wall -Wextra -Werror, zero sanitizer findings; gcc 13.3 -O2 disassembly of the implementation is mov/imul/shr/ret with no div/idiv; measured at -O2: 2.221 ns/value (450.3 Mvalues/s, best of 5, PRNG pre-generated, excluded from the timed loop)
- **lab/90**: bit-reversed index permutation of an 8-element array (out[bitrev(i)] = in[i]), the 3-bit reversal built only from the swap identities, no lookup table, no library call; 1,000,000 randomized 8-element distinct-value arrays (xorshift64* seed 0x9E3779B97F4A7C15), 0 mismatches vs an independent explicit per-bit-loop reference, bijection holds (every value appears exactly once) on every case, involution (perm applied twice restores the original order) holds on every case, FNV-1a checksum 0x43d798a1ea296710 identical across -O2/-O0/ASan+UBSan, zero warnings under -std=c11 -Wall -Wextra -Werror, zero sanitizer findings; -O2 disassembly of bitrev8_perm is only shift/mask/or/compare plus loads/stores, with no .rodata table load and no library reversal call; measured at -O2: 1.35 ns/value (best of 5, PRNG pre-generated, excluded from the timed loop)
- **lab/93**: count of zero bytes in a 64-bit word via the zero-byte detection identity ((x - 0x0101..01) & ~x & 0x8080..80); the naive form overcounts because a borrow out of a zero low byte can set a phantom 0x80 bit in the byte above (measured: 1001 overcounts in 10,065,536 inputs), so the module runs the subtraction in 16-bit lanes with a 0x7F guard per lane and no borrow ever crosses a lane boundary; 10,065,536 differential checks (all 65,536 16-bit inputs + 10,000,000 fixed-seed splitmix64 words, seed 0x123456789ABCDEF0) vs a per-byte reference, 0 mismatches; FNV-1a checksum ca180bdf75d1ddd9 identical across -O0/-O2/ASan+UBSan, zero warnings under -std=c11 -Wall -Wextra -Werror, zero sanitizer findings; -O2 disassembly of the hot path is straight-line integer arithmetic, no byte-by-byte loop; measured at -O2: 2.244 ns/value (best of 5, PRNG pre-generated, excluded from the timed loop)
- **lab/94**: high 64 bits of the signed 128-bit product of two int64 values: from a = ua - sa*2^64, b = ub - sb*2^64, the high word is exactly mulhi_u64(ua,ub) - sa*ub - sb*ua (mod 2^64), the unsigned high word from the 32-bit splitting identity and the sign corrections as conditional subtracts; no __int128 and no full-width signed multiply in the implementation (grep-verified), signed __int128 appears only in the test oracle; 4,304,979,732 differential checks per build (all 2^32 sign-extended 16-bit pairs + 10,000,000 fixed-seed splitmix64 pairs, seed 0x123456789ABCDEF0 + 12,436 directed cases incl. INT64_MIN x INT64_MIN, INT64_MIN x -1, (-1) x (-1)), 0 mismatches across -O0/-O2/ASan+UBSan; FNV-1a checksum 0x3282d931333f6a85 identical across all three builds, zero warnings under -std=c11 -Wall -Wextra -Werror, zero sanitizer findings; -O2 disassembly is four 32-bit half-width imuls with carry folding (add/setb/adc) and branchless test/cmovs sign corrections, no 64x64 multiply of the original operands; measured at -O2: 5.942 ns/pair (168.3 Mpairs/s, best of 5, PRNG pre-generated, excluded from the timed loop)
- **lab/92**: branchless select(sel, a, b) of two uint64 values from the mask identity (mask = -(uint64_t)sel, out = (a & ~mask) | (b & mask)); 8,599,934,601 differential checks (9 directed + 8,589,934,592 exhaustive 16-bit triples + 10,000,000 fixed-seed splitmix64 64-bit triples, seed 0x123456789ABCDEF0) vs the ternary operator, 0 mismatches; FNV-1a checksum 47f9a66e9c481f08 identical across -O0/-O2/ASan+UBSan, zero warnings under -std=c11 -Wall -Wextra -Werror, zero sanitizer findings; programmatic scan of the -O2 object found 0 jump instructions; measured at -O2: 2.68 ns/value (372.5 Mvalues/s, best of 5)
- **lab/107 (saturate-to-u16)**: branchless clamp of a signed 32-bit value into [0, 65535] from the sign-mask identities; exhaustive 2^32 inputs differential-tested against an if/else reference, 4,294,967,296 cases, 0 mismatches; monotonicity and idempotence held on every case; -O2 disassembly contains 0 conditional jumps; measured at -O2: 0.694 ns/value (1440.114 M values/s, best of 5, PRNG pre-generated)
- **lab/95**: horizontal sum of four packed unsigned 16-bit lanes in one 64-bit word from the pairwise-add cascade identity (stage 1 sums even/odd lanes with the 0x0000FFFF0000FFFF guard mask into 32-bit fields, stage 2 folds the pair); 4,304,967,310 differential checks (14 directed words + 4,294,967,296 exhaustive (a,b,a,b) replicated lane pairs + 10,000,000 fixed-seed splitmix64 words, seed 0x123456789ABCDEF0) vs an independent scalar lane-shift-and-add reference, 0 mismatches; lane-crosstalk absence proved from the guard bounds (each pair sum at most 0x1FFFE, below 2^32); FNV-1a checksum 0eb4f4d7484f98bd identical across -O0/-O2/ASan+UBSan, zero warnings under -std=c11 -Wall -Wextra -Werror, zero sanitizer findings; measured at -O2: 2.21 ns/value (452.1 Mvalues/s over 25M timed values, best of 5, PRNG pre-generated, excluded from the timed loop)
- **lab/28**: round-half-to-even u32 halving `tie_to_even_half(x)` from the bit identity `q + ((x & 1) & (q & 1))` with `q = x >> 1` (no floats, no division); exhaustive over ALL 4,294,967,296 u32 values plus 13 hand-derived contract rows (ties to even: 1 -> 0, 3 -> 2, 5 -> 2, 7 -> 4, plus 2^31 and 2^32-1 boundary rows), 0 mismatches vs an independent quotient/remainder division-based reference; FNV-1a checksum 0x5ae86dd58b5d3a01 identical across -O0/-O2/ASan+UBSan, zero warnings under -std=c11 -Wall -Wextra -Werror, zero sanitizer findings; gcc 13.3 -O2 disassembly is exactly shr/and/and/add/ret with no div/idiv; measured at -O2: 2.29 ns/value (best of 5, PRNG pre-generated, excluded from the timed loop)
- **lab/85**: two-word (128-bit) addition with carry propagation, each stage from the carry-out identity carry = (sum < a) | (cin & (sum == a)), where the (cin & (sum == a)) term is the wrap detector for the b == UINT64_MAX edge; no __int128 in the implementation (oracle only); 8,590,934,616 differential checks (24 directed edge rows + exhaustive 16-bit sweep with carry_in 0/1 per case + 1,000,000 fixed-seed splitmix64 full-width triples, seed 20260910), 0 mismatches vs unsigned __int128, final carry-out checked on every case; FNV-1a checksum 0xd0a64bebc1998760 identical across -O0/-O2/ASan+UBSan, zero warnings under -std=c11 -Wall -Wextra -Werror, zero sanitizer findings; measured at -O2: 12.526 ns/value (200M timed values)
- **lab/86**: saturating 32-bit addition from the overflow-flag identity (result = a + b if no overflow, else INT32_MAX/MIN); 1,065,547 differential checks (11 known-answer + 1,000,000 fixed-seed splitmix64 pairs + 65,536 directed edge pairs), 0 mismatches; FNV-1a checksum 0x6e1b8f0f3391f907 identical across -O0/-O2/ASan+UBSan, zero warnings under -std=c11 -Wall -Wextra -Werror, zero sanitizer findings; measured at -O2: 5.146 ns/pair (best of 5)
- **lab/100**: branchless three-way unsigned 64-bit compare bcmp64(a, b) returning -1/0/+1 from the borrow-out identity (lt = (a - b) > a, eq = (a == b), result = gt - lt); 4,304,967,310 differential checks (14 directed rows incl. UINT64_MAX and the signedness-boundary rows + 4,294,967,296 exhaustive 16-bit pairs + 10,000,000 fixed-seed splitmix64 64-bit pairs, seed 0x123456789ABCDEF0) vs (a > b) - (a < b), 0 mismatches; FNV-1a checksum 07ae4885650dd8ca identical across -O0/-O2/ASan+UBSan, zero warnings under -std=c11 -Wall -Wextra -Werror, zero sanitizer findings; programmatic scan of the gcc 13.3 -O2 object found 0 jump instructions (the compiler folds it to cmp/setb/sete, reading the hardware borrow flag directly); measured at -O2: 2.31 ns/value (432.7 Mvalues/s over 25M timed values, best of 5)
- **lab/104**: high 16 bits of the 32-bit product of two `uint16_t` values, `mulhi_u16(a, b)`, from the schoolbook shift-add partial-product identity: the multiplier's bits select shifted copies of the multiplicand into a 32-bit accumulator, returning acc >> 16, with no multiply operator anywhere in the implementation (not even a 32-bit one); 4,294,967,673 differential checks per build (377 directed edge cases as an 11x11 boundary cross product plus all 16x16 power-of-two pairs, plus all 4,294,967,296 exhaustive 16-bit pairs, the complete input space) vs the native 32-bit product oracle confined to the test file, 0 mismatches across -O0, -O2, and ASan+UBSan; FNV-1a checksum 0xbe84b05c63e3bea1 identical across all three builds, zero warnings under -std=c11 -Wall -Wextra -Werror, zero sanitizer findings; gcc -O2 disassembly of the implementation contains no mul/imul (the shift-add loop compiled branchless: test/lea/cmovne, `add edi,edi` to double x, `shr si,1` to walk the multiplier bits), the `make disasm` step fails the build if one appears; measured at -O2: 11.807 ns/pair (best of 5, 1M timed pairs, PRNG pre-generated, excluded from the timed loop)
