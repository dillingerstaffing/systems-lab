...
- **lab/126**: low 16 bits of a 16x16 multiply from shift-add over the set bits of one operand, no multiply operator anywhere (verified in the -O2 disassembly), differential-tested over the full 2^32 input space ([lab/126-mul-lo16-nomul](lab/126-mul-lo16-nomul)).
- **lab/128**: low 32 bits of a 64x64 multiply from 16-bit half-word schoolbook partial products, keeping only the terms that reach the low 32 bits (the a1*b1 term vanishes mod 2^32), no multiply operator anywhere (verified in the -O2 disassembly) ([lab/128-mul-lo32-nomul](lab/128-mul-lo32-nomul)).
...
- **lab/115**: count-leading-zeros clz64_fp(x) via the IEEE-754 binary64 exponent field: fill x upward to 2^(n+1)-1, read the unbiased exponent through a union, clz = 63 - that, x = 0 pinned to 64; the rounding hazard (filled values >= 2^53 round up one exponent step) is derived and corrected (x >> 53 subtracts 1); 10,065,537 differential checks vs __builtin_clzll, 0 mismatches, FNV-1a bb15711279c99def identical across -O0/-O2/ASan+UBSan.
- **lab/116**: software parallel-bit-deposit pdep64(src, mask) scattering the low popcount(mask) bits of src into the set mask positions via the loop-scatter identity with lowest-bit isolation m & (~m + 1); 1,002,060 differential checks vs a naive per-bit loop, 0 mismatches, popcount conservation invariant on every case, FNV-1a 5780cdf6a1013eb2 identical across -O0/-O2/ASan+UBSan.
- **lab/108**: count-leading-zeros clz64_halving(x) from the binary-search halving identity (for halves k in 32/16/8/4/2/1: if (x >> (64 - k)) == 0 then n += k, x <<= k); 10,065,537 differential checks vs __builtin_clzll (exhaustive 16-bit plus 10M fixed-seed splitmix64 values), 0 mismatches, FNV-1a bb15711279c99def identical across -O0/-O2/ASan+UBSan, x = 0 pinned to 64 by contract; the -O2 disassembly is verified to contain no clz-class instruction (shr/shl/test/add/mov/not only), 7.56 ns/value at -O2 (best of 5).
- **lab/101**: round-to-nearest-even of a 64-bit fixed-point value from the bit-manipulation identity (no float, no division); halfway cases tie to even by the low-bit rule; 1,065,547 differential checks, 0 mismatches, FNV-1a 258f283508816c19, 2.66 ns/value at -O2.
- **lab/109**: saturating negation of int64 neg_sat64(x) (neg_sat(INT64_MIN) = INT64_MAX, else -x) from the sign-mask identity neg = s - u + ((s & 1) ^ e) with s = x >> 63 and e the branchless INT64_MIN detector from the d | (-d) bit-63 identity, no branch and no comparison; 10,065,537 differential checks vs the ternary reference (exhaustive signed 16-bit plus 10M fixed-seed splitmix64 values plus the dedicated INT64_MIN row), 0 mismatches, FNV-1a e52a81b464c7559c identical across -O0/-O2/ASan+UBSan; the -O2 disassembly contains no conditional jump, 6.21 ns/value at -O2 (best of 5).
- **lab/125**: unsigned 32-bit division by 7 udiv7_shiftadd(x) from the binary shift-add series (1/7 = 0.001001001... repeating in binary, q0 = sum of (x >> 3k) for k=1..10, then at most 10 shift-based correction steps), no division or multiply anywhere in the implementation; 26,777,238 differential checks vs the / operator (exhaustive 24-bit inputs plus 10M fixed-seed 32-bit values plus 22 edge cases), 0 mismatches, FNV-1a 835b51585dc7f434 identical across -O0/-O2/ASan+UBSan; the -O2 disassembly contains no div/idiv, 16.600 ns/value at -O2 (best of 5).
- **lab/110**: fixed 8-bit bit permutation P = [2,5,0,7,1,6,3,4] as the unrolled shift/mask sum OR_i (((b >> i) & 1) << P[i]) with the explicit inverse Q = [2,4,0,6,7,1,5,3] derived as the exact index reversal of P (Q[P[i]] = i for all i); perm16/perm64 apply it per byte; 4,524,321 differential checks vs naive per-bit loop references (33 mapping checks, exhaustive 65,536 16-bit inputs, 1M fixed-seed splitmix64 64-bit values), 0 mismatches, inv(perm(x)) == x and perm(inv(x)) == x on every case, FNV-1a f62cbb51d7116999 identical across -O0/-O2/ASan+UBSan, 39.464 ns/value at -O2 (best of 5).
- **lab/121**: branchless conditional negation cond_neg(x, f) = (x ^ -f) + f for 64-bit x and flag f in {0,1}, from the two's-complement identity (XOR with all-ones then +1 negates); 10,131,084 differential checks vs the if/else reference (131,072 exhaustive: all 65,536 16-bit x values times both flag values, plus 10,000,000 fixed-seed splitmix64 pairs, plus 12 edge rows), 0 mismatches, FNV-1a 1743a170968927e0 identical across -O0/-O2/ASan+UBSan with zero sanitizer reports; the -O2 disassembly is five instructions (mov/neg/xor/add/ret) with 0 jump instructions, 4.37 ns/value at -O2 (best of 5).
- **lab/122**: decimal digit count digit_count(x) of a 64-bit word as the straight-line comparison cascade 1 + (x >= 10) + ... + (x >= 10^19), no loop, no division, no string formatting ([lab/122-decimal-digits](lab/122-decimal-digits)).
- **lab/124**: branchless power-of-two test is_pow2(x) = (x != 0) & ((x & (x - 1)) == 0), clearing the lowest set bit leaves zero exactly for powers of two, -O2 disassembly contains no conditional jumps ([lab/124-is-pow2](lab/124-is-pow2)).
- **lab/118**: single-bit set/clear/toggle/test of a 64-bit word from the (1ULL << i) mask identities, with the i = 63 edge pinned and no shift by 64 anywhere ([lab/118-bit-single](lab/118-bit-single)).
- **lab/132**: Sv39 virtual-address field extraction vpn_extract(va) from the shift/mask identities, with recombine(extract(va)) == va checked on every case against a per-bit oracle ([lab/132-sv39-vpn-extract](lab/132-sv39-vpn-extract)).
- **lab/133**: Sv39 page-table entry decode pte_decode(pte) into PPN and flag bits from the shift/mask identities, with encode(decode(pte)) == pte checked on every case against a per-field reference ([lab/133-sv39-pte-decode](lab/133-sv39-pte-decode)).
- **lab/134**: RV64 satp CSR field extraction (MODE, ASID, PPN) from the shift/mask identities, with recombine(extract(s)) == s checked on every case against a per-field reference ([lab/134-satp-ppn-extract](lab/134-satp-ppn-extract)).
- **lab/137**: RV64 satp.MODE legality validator, satp_mode_legal_rv64(mode) answering whether a 4-bit MODE value names a defined translation scheme (0 Bare, 8 Sv39, 9 Sv48, 10 Sv57 per Table 114), differential-tested against a hand-transcribed spec-table oracle over the exhaustive value space ([lab/137-satp-mode-validate](lab/137-satp-mode-validate)).
- **lab/135**: Sv39 leaf-PTE access-permission check pte_permits(r, w, x, u, access, mode) from the privileged spec's R/W/X/U rules, checked against a separately written spec-table reference over all flag/access/mode combinations ([lab/135-pte-permission-check](lab/135-pte-permission-check)).
- **lab/136**: Sv39 physical-address formation phys_addr_from_ppn(ppn, offset) = (ppn << 12) | offset from the shift/OR identities, checked on every case against a per-bit oracle ([lab/136-pte-to-phys-addr](lab/136-pte-to-phys-addr)).
- **lab/144**: Sv39 leaf-PTE permission rules from the privileged spec's R/W/X/U encodings, differential-tested against an independent oracle ([lab/144-pte-permission-check](lab/144-pte-permission-check)).
- **lab/149**: Sv39 multi-level walk permission check, walk_permits(levels, nlevels, access, mode) following the spec's rule that permission is decided at the terminating leaf while non-leaf entries must be pure pointers, differential-tested against an independent oracle over all level-bit/access/mode combinations ([lab/149-pte-permission-and](lab/149-pte-permission-and)).
- **lab/146**: Sv39 virtual-address canonicality check from the sign-extension identity (bits 63:39 all equal bit 38), differential-tested against a per-bit reference over directed sign-boundary rows plus random values ([lab/146-va-canonical-check](lab/146-va-canonical-check)).
- **lab/146-pte-accessed-bit**: Sv39 leaf-PTE accessed-bit rule check pte_a_check(flags, access) from the privileged spec's A-bit rules (a permitted access sets A=1, a faulting access leaves the PTE untouched), differential-tested against a hand-transcribed spec-table oracle ([lab/146-pte-accessed-bit](lab/146-pte-accessed-bit)).
- **lab/147-pte-global-bit**: Sv39 leaf-PTE global-bit rule check from the privileged spec's G-bit rules, with an ASID-tagged translation cache encoding the rule that a global entry matches any ASID and a non-global entry matches only its own ASID, differential-tested against an independently written oracle ([lab/147-pte-global-bit](lab/147-pte-global-bit)).
- **lab/150-pte-a-d-bits**: Sv39 walk-level accessed/dirty prediction, ad_predict(tree, va, access, mode) predicting exactly which PTE gains A and which gains D over scripted access sequences on scripted page-table trees, following the spec's rule that the hardware update applies to the terminating leaf PTE only, differential-tested against an independently written oracle ([lab/150-pte-a-d-bits](lab/150-pte-a-d-bits)).
- **lab/152-pte-superpage-misalign**: Sv39 superpage misalignment check, pte_superpage_misaligned(level, ppn) reporting whether a leaf PTE's low PPN bits must raise a page fault for a 2 MiB / 1 GiB superpage at level 1 / 2, with level 0 never faulting and out-of-domain levels reported explicitly, differential-tested against an independent per-bit oracle ([lab/152-pte-superpage-misalign](lab/152-pte-superpage-misalign)).
- **lab/155-sv39-nonleaf-ppn-ignored**: Sv39 single-step walk classification, pte_step(pte, level) reporting FAULT, DESCEND, or LEAF for one PTE per the privileged spec's walk steps 3 and 4, correcting the backlog gloss (the non-leaf PPN is the next-level pointer and is always read; non-leaf D/A/U fault via step 3; the genuinely unread bits for the verdict are G and RSW), differential-tested against an independent divmod bit-array oracle ([lab/155-sv39-nonleaf-ppn-ignored](lab/155-sv39-nonleaf-ppn-ignored)).
- lab/155-sv39-nonleaf-ppn-ignored: Sv39 PTE decode proving the correct non-leaf ignore rule (G/RSW bits ignored for verdict), link: https://github.com/dillingerstaffing/systems-lab/tree/main/lab/155-sv39-nonleaf-ppn-ignored
- **lab/156-pidfd-getfd**: `pidfd_getfd(2)` firsthand, the parent reaching into the child's file-descriptor table and taking one: the child opens an unlinked 20-byte tmpfile, seeks to offset 5, and the parent reads `"56789ABCDEFGHIJ"` through the duplicated descriptor with no seek of its own, proving the shared open file description; returned descriptor arrives `FD_CLOEXEC`, `flags=1` refused with `EINVAL`, 3/3 runs byte-identical ([lab/156-pidfd-getfd](lab/156-pidfd-getfd)).
- **lab/157-tls**: thread-local storage firsthand: a `__thread int counter = 7` shows two threads two different addresses (same name), the worker's store of 42 leaves main's 7 untouched, and `errno` behaves the same per thread (worker 5, main 0); 3 runs x 7 assertions, 0 mismatches; disassembly shows the initial-exec model as one instruction (`mov %fs:0xfffffffffffffffc,%ecx`), the shared-library general-dynamic model as `lea` + `call __tls_get_addr@plt`, and the RISC-V lowering as `lui`/`add tp`/`lw`; `readelf` shows the `PT_TLS` template ([lab/157-tls](lab/157-tls)).
- **lab/158-kalloc**: xv6's physical page allocator firsthand: the real `kalloc.c` (verbatim from mit-pdos/xv6-riscv master) compiled for the host against a 64 MiB fake arena; `freerange` drains exactly 16,383 distinct aligned pages, the freelist is a stack (free a,b,c then kalloc returns c,b,a), `kfree` fills with 0x01 and `kalloc` refills with 0x05, `kfree` panics on a misaligned address, an address below `end`, and an address >= `PHYSTOP`, and 4 threads x 3,000 alloc/free cycles conserve all pages with acquire/release balanced at 89,553; 3 runs x 12 assertions, 0 mismatches, byte-identical ([lab/158-kalloc](lab/158-kalloc)).
- **lab/159-svadu-adue**: Svadu firsthand on bare metal: `menvcfg.ADUE` (bit 61) decides who writes the A and D bits in a PTE; with ADUE=0 a store traps exactly once (`scause` 15) and software sets A and D (PTE `0x200400c7`), a load traps exactly once (`scause` 13) and software sets A only (PTE `0x20040047`); with ADUE=1 the walker sets the bits itself with an atomic update and zero traps fire; the test runs in real S-mode with faults delegated via `medeleg`, ADUE flipped by M-mode through `ecall`; 3 runs x 4 assertions, 0 mismatches, byte-identical ([lab/159-svadu-adue](lab/159-svadu-adue)).
- **lab/160-sync-file-range**: `sync_file_range(2)` firsthand: the syscall syncs a byte range, not a file, and its flags split writeback into start and wait; `flags=0` no-op returns 0, a bad flag bit returns `EINVAL`, a pipe fd returns `ESPIPE`, `WRITE` alone returns after only starting writeback while a separate `WAIT_AFTER` waits, the full `WAIT_BEFORE|WRITE|WAIT_AFTER` combination on bytes `[2MiB,6MiB)` returns 0, and the full-range combination beats `fsync` on the same dirty file 3/3 iterations because `fsync` also pays metadata and journal; 3 runs x 12 assertions, 0 mismatches, digit-normalized logs checksum-stable ([lab/160-sync-file-range](lab/160-sync-file-range)).
- **lab/161-ztso-fence-delete**: Ztso (Total Store Ordering) firsthand through the compiler: `tso.c` holds four atomics compiled for `rv64gc` and `rv64gc_ztso` (xpack GCC 15.2.0, -O2); the Ztso build deletes exactly the fences the spec makes redundant (`fence rw,w` before a seq_cst store, `fence r,rw` after an acquire load), keeps one `fence rw,rw` per seq_cst op, and leaves release stores and acquire loads fully bare; the ztso object's `Tag_RISCV_arch` gains `_ztso1p0`, the base object's does not; 3 runs x 12 assertions, 0 mismatches, byte-identical ([lab/161-ztso-fence-delete](lab/161-ztso-fence-delete)).
- **lab/119**: 16-bit one's-complement adder with end-around carry from the wraparound identity (the single lost carry bit is `s < a` after the 16-bit wrap), isolated from any checksum application, differential-tested against the loop-fold reference ([lab/119-onesc-add](lab/119-onesc-add)).
- **lab/127**: branchless saturating 64-bit left shift, overflow detected exactly as nonzero bits shifted out ((x >> (64-k)) != 0) and merged with a 0/1 mask, disassembly scan confirming no conditional jump ([lab/127-sat-shl](lab/127-sat-shl)).
- **lab/140**: high 64 bits of the unsigned 128-bit product from the 32-bit splitting identity (four 32x32 partial products with explicit carry accounting, no 128-bit type in the implementation), differential-tested against an exact unsigned __int128 reference ([lab/140-mulhi-unsigned](lab/140-mulhi-unsigned)).
- **lab/162-bottomhalf-deferred**: Linux deferred interrupt work read firsthand from vendored v6.5 kernel sources: ten softirq types in priority order (RCU last), the 2ms and 10-restart governors found by experimentation, the `ksoftirqd` handoff when the budgets are spent, dispatch only when `!in_interrupt()`, and the tasklet SCHED/RUN test-and-set contract with re-queue on contention; 24 line-pinned fact checks, 0 mismatches ([lab/162-bottomhalf-deferred](lab/162-bottomhalf-deferred)).
