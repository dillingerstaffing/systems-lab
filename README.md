...
- **lab/146-pte-accessed-bit**: Sv39 leaf-PTE accessed-bit rule check pte_a_check(flags, access) from the privileged spec's A-bit rules (hardware sets A=1 on a permitted access, faults leave the PTE untouched), differential-tested against a hand-transcribed 3x16 spec-table oracle over the full space (all 256 flag-byte values times 3 access types, 768 cases), 2304 checks, 0 mismatches, FNV-1a 75d52de33bd90675 identical across -O0/-O2/ASan+UBSan, 3.443 ns/value at -O2 (best of 5) ([lab/146-pte-accessed-bit](lab/146-pte-accessed-bit)).
