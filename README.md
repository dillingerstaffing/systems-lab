...
- **lab/132**: Sv39 virtual-address field extraction vpn_extract(va) from the shift/mask identities (vpn[i] = (va >> (12 + 9*i)) & 0x1FF, page_offset = va & 0xFFF); the four masks are disjoint and cover bits 38:0 with no overlap, so recombine(extract(va)) == va on every case, differential-tested against a per-bit oracle over 26,777,374 cases with 0 mismatches ([lab/132-sv39-vpn-extract](lab/132-sv39-vpn-extract)).
